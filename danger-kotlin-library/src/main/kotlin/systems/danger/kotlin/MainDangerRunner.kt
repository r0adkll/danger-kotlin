 ...
 * @param jsonOutputPath the output json file path used to publish the danger results on your Pull
 *   Request
 * @constructor Creates the main DangerContext
 */
internal class MainDangerRunner(jsonInputFilePath: FilePath, jsonOutputPath: FilePath) :
  DangerContext {
  ...
  private fun AtomicReference<MutableList<Violation>>.updateWith(violation: Violation) {
    this.getAndUpdate { it.apply { add(violation) } }
    commit()
  }
