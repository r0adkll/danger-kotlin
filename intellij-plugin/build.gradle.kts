...
intellijPlatform {
  pluginConfiguration {
    ...
    changeNotes =
      providers.gradleProperty("VERSION_NAME").map { pluginVersion ->
        with(changelog) {
          renderItem(
  ...
  publishing {
    ...
    channels =
      providers.gradleProperty("VERSION_NAME").map {
...
tasks.register<Copy>("copyDangerJar") {
  dependsOn(":danger-kotlin-library:shadowJar")
  from(tasks.getByPath(":danger-kotlin-library:shadowJar").outputs)
  into("build/resources/main/jar")
  include("*.jar")
}
